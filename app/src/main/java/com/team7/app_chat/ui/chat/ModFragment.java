...
public class ModFragment extends Fragment implements MemberAdapter.IBottomSheet, ContactModalBottomSheet.INavBottomSheet {
  ...
  public ModFragment(String roomId) {
  ...
  private void loadData(boolean isMod){
    ...
    roomRepository.get().document(roomId)
        .collection("members")
        .whereEqualTo("mod", true)
        .addSnapshotListener((value, error) -> {
          if(error != null) return;
